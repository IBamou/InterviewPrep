            ...
            <div class="bg-white border border-outline-variant/50 rounded-xl p-5 space-y-5">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="color" id="color-picker" value="{{ old('color', '#6366f1') }}" class="w-9 h-9 rounded border border-outline-variant/60 cursor-pointer p-0.5" oninput="document.getElementById('color').value=this.value"/>
                        <input class="w-24 h-9 px-3 rounded-lg border border-outline-variant/60 text-[13px] font-mono focus:border-primary focus:ring-2 focus:ring-primary/15 outline-none transition-all @error('color') border-error @enderror" id="color" name="color" value="{{ old('color', '#6366f1') }}" type="text" placeholder="#6366f1" maxlength="7" oninput="document.getElementById('color-picker').value=this.value"/>
                    </div>
